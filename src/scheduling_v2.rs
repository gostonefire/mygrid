...
const SOC_KWH: f64 = BAT_CAPACITY / 100.0;
...
impl fmt::Display for Block {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        ...
        // Build base output
        let output = format!("{} - {} -> {:>2} - {:>2}: SocIn {:<3}, SocOut {:<3}, chargeIn {:<5.2}, chargeOut {:<5.2}, chargeTariffIn {:<5.2}, chargeTariffOut {:<5.2}, overflow {:<5.2}, overflowPrice {:<5.2}, updated: {:>5}, price {:<5.2} ",
...
impl Schedule {
    ...
    pub fn new_with_scheduling(start: usize, tariffs: &Vec<f64>, production: &PVProduction, consumption: &Consumption, charge_in: f64, charge_tariff_in: f64, date_time: DateTime<Local>) -> Schedule {
        ...
        let blocks = seek_best(start, &tariffs, net_prod, charge_in, charge_tariff_in);
...
/// * 'charge_in' - any residual charge to bear in to the new schedule
/// * 'charge_tariff_in' - the mean price for the residual price
fn seek_best(start: usize, tariffs: &Tariffs, net_prod: [f64;24], charge_in: f64, charge_tariff_in: f64) -> Blocks {
    let mut schedule_id: u32 = 0;
    let mut record: HashMap<usize, Blocks> = create_base_blocks(schedule_id, charge_in, charge_tariff_in, &tariffs, net_prod);

    for seek_first_charge in start..23 {
        for charge_level_first in 0..=90 {
            schedule_id += 1;

            let first_charge_blocks = seek_charge(start, seek_first_charge, charge_level_first, &tariffs, net_prod, charge_in, charge_tariff_in);
            for seek_first_use in first_charge_blocks.next_start..24 {

                if let Some(first_use_blocks) = seek_use(schedule_id, first_charge_blocks.next_start, seek_first_use, &tariffs, net_prod, first_charge_blocks.next_charge_in, first_charge_blocks.next_charge_tariff_in) {
                    let first_combined = combine_blocks(&first_charge_blocks, &first_use_blocks);
                    record_best(1, &first_combined, &tariffs, net_prod, &mut record);

                    for seek_second_charge in first_combined.next_start..23 {
                        for charge_level_second in 0..=90 {
                            schedule_id += 1;

                            let second_charge_blocks = seek_charge(first_combined.next_start, seek_second_charge, charge_level_second, &tariffs, net_prod, first_combined.next_charge_in, first_combined.next_charge_tariff_in);
                            for seek_second_use in second_charge_blocks.next_start..24 {

                                if let Some(second_use_blocks) = seek_use(schedule_id, second_charge_blocks.next_start, seek_second_use, &tariffs, net_prod, second_charge_blocks.next_charge_in, second_charge_blocks.next_charge_tariff_in) {
                                    ...
                                    record_best(2, &all_combined, &tariffs, net_prod, &mut record);
                                }
                            }
                        }
                    }
...
/// * 'net_prod' - the net production (production minus consumption) per hour
fn create_base_blocks(schedule_id: u32, charge_in: f64, charge_tariff_in: f64, tariffs: &Tariffs, net_prod: [f64;24]) -> HashMap<usize, Blocks> {
...
/// * 'net_prod' - the net production (production minus consumption) per hour
fn trim_and_tail(blocks: &Blocks, tariffs: &Tariffs, net_prod: [f64;24]) -> Blocks {
...
/// * 'net_prod' - the net production (production minus consumption) per hour
/// * 'record' - the record of the best Blocks structs saved so far
fn record_best(level: usize, blocks: &Blocks, tariffs: &Tariffs, net_prod: [f64;24], record: &mut HashMap<usize, Blocks>) {
    if let Some(recorded_blocks) = record.get(&level) {
        if blocks.total_price > recorded_blocks.total_price {
            record.insert(level, trim_and_tail(blocks, tariffs, net_prod));
        }
    } else {
        record.insert(level, trim_and_tail(blocks, tariffs, net_prod));
    }
...
/// * 'charge_tariff_in' - mean tariff for residual charge
fn seek_use(schedule_id: u32, initial_start: usize, seek_start: usize, tariffs: &Tariffs, net_prod: [f64;24], charge_in: f64, charge_tariff_in: f64) -> Option<Blocks> {
    ...
    for u_start in seek_start..24 {
        ...
        for u_end in u_start..=24 {
            if u_end > 23 || tariffs.buy[u_end] <= charge_tariff_out {
                if u_end != u_start {
                    return Some(get_use_blocks(schedule_id, u_start, u_end, charge_out, charge_tariff_out, overflow, overflow_price, hold, tariffs, net_prod));
                }
                break;
            }
            ...
            if charge_out.round() == 0.0 {
                if u_end != u_start {
                    return Some(get_use_blocks(schedule_id, u_start, u_end + 1, charge_out, charge_tariff_out, overflow, overflow_price, hold, tariffs, net_prod));
                }
                break;
            }
        }
...
fn get_charge_block(start: usize, size: usize, charge_in: f64, charge_tariff_in: f64, charge_out: f64, charge_tariff_out: f64, price: f64) -> Block {
    Block {
...
/// * 'net_prod' - the net production (production minus consumption) per hour
fn get_use_blocks(schedule_id: u32, u_start: usize, u_end: usize, charge_out: f64, charge_tariff_out: f64, overflow: f64, overflow_price: f64, hold: Block, tariffs: &Tariffs, net_prod: [f64;24]) -> Blocks {
...
/// * 'charge_in' - residual charge from previous block
/// * 'charge_tariff_in' - mean tariff for residual charge
fn seek_charge(initial_start: usize, start: usize, soc_level: usize, tariffs: &Tariffs, net_prod: [f64;24], charge_in: f64, charge_tariff_in: f64) -> Blocks {
    ...
    let charge: Block = if need > 0.0 {
        ...
        get_charge_block(start, end - start, hold_charge_out, hold_charge_tariff_out, hold_charge_out + need, c_mean, c_price)

    } else {
        get_charge_block(start, 0, hold_charge_out, hold_charge_tariff_out, hold_charge_out, hold_charge_tariff_out, 0.0)
    };
...
pub fn create_new_schedule(nordpool: &NordPool, smhi: &mut SMHI, date_time: DateTime<Local>, backup_dir: &str) -> Result<Schedule, SchedulingError> {
    ...
    let schedule = Schedule::new_with_scheduling(date_time.hour() as usize, &tariffs, &production, &consumption, 0.0, 0.0, date_time);
