...
impl PVProduction {
    ...
    /// Calculate estimates for the days included in the forecast vector
    ///
    /// # Arguments
    ///
    /// * 'forecast' - a vector of hourly weather forecasts
    pub fn estimate(&self, forecast: &Vec<ForecastValues>) -> Result<(Vec<ProductionValues>, Vec<ProductionValues>), ProdError> {
        let dates = forecast.iter().fold(Vec::<DateTime<Local>>::new(), |mut acc, f| {
            let date = f.valid_time.duration_trunc(TimeDelta::days(1)).unwrap();
            if !acc.contains(&date) {
                acc.push(date);
            }
            acc
        });

        let mut five_intrvl: Vec<ProductionValues> = Vec::new();
        let mut hourly_intrvl: Vec<ProductionValues> = Vec::new();

        for date in dates {
            let temp = minute_values(forecast, date, |f| f.temp)?;
            let cloud_factor = minute_values(forecast, date, |f| f.cloud_factor)?;
            let power = self.day_power(date, temp, cloud_factor)?;

            five_intrvl.extend(group_on_time(power, date, 5));
            hourly_intrvl.extend(group_on_time(power, date, 60));
        }

        Ok((hourly_intrvl, five_intrvl))
    }
...
fn minute_values(forecast: &Vec<ForecastValues>, date_time: DateTime<Local>, y_fn: fn(&ForecastValues) -> f64) -> Result<[f64;1440], ProdError> {
    let xy = forecast
        .iter()
...
fn group_on_time(data: [f64;1440], date_time: DateTime<Local>, group: u32) -> Vec<ProductionValues> {
