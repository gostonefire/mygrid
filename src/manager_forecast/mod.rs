...
use crate::models::forecast::{ForecastRecord, ForecastValues};
...
impl Forecast {
    ...
    /// Retrieves a weather forecast for the given date (plus one)
    ///
    /// # Arguments
    ///
    /// * 'date_time' - the date to get a forecast for
    pub fn new_forecast(&self, date_time: DateTime<Local>) -> Result<Vec<ForecastValues>, ForecastError> {
        let from = date_time.duration_trunc(TimeDelta::days(1)).unwrap();
        let to = from.add(TimeDelta::days(1)).with_hour(23).unwrap();
